...
static QVector<QPointF>temp;
const double TIME_MAX=10.0;//时间刻度最大值
extern int div_time;
//double div_time=20.0;//发送命令的最小单位；
...
bool GraphicsItem::readBaseAttributes(QXmlStreamReader *xml)
{
    ...
    setRotation(xml->attributes().value("rotate").toDouble());
    setPos(x,y);
    QString color=xml->attributes().value(tr("color")).toString();
    QColor clr(color);
    QPen pen;
    pen.setColor(clr);
    this->setPen(pen);
    m_servoid=xml->attributes().value(tr("id")).toString();
    return true;
}
...
bool GraphicsItem::writeBaseAttributes(QXmlStreamWriter *xml)
{
    ...
    xml->writeAttribute("color",QString("%1").arg(this->penColor().name()));
    xml->writeAttribute(tr("id"),QString("%1").arg(this->m_servoid));
    return true;
}

//bool GraphicsItem::readBaseAttributes(QXmlStreamReader *xml)
//{
//    m_actionid=xml->attributes().value("actionGroupID").toString();
//    m_servoid=xml->attributes().value("servoID").toString();
//    qreal x = xml->attributes().value(tr("x")).toDouble();
//    qreal y = xml->attributes().value(tr("y")).toDouble();
//    m_width = xml->attributes().value("width").toDouble();
//    m_height = xml->attributes().value("height").toDouble();
...
//bool GraphicsItem::writeBaseAttributes(QXmlStreamWriter *xml)
//{
//    xml->writeAttribute(tr("actionGroupID"),QString("%1").arg(m_actionid));
//    xml->writeAttribute(tr("servoID"),QString("%1").arg(m_servoid));
//    xml->writeAttribute(tr("rotate"),QString("%1").arg(rotation()));
//    xml->writeAttribute(tr("x"),QString("%1").arg(pos().x()));
//    xml->writeAttribute(tr("y"),QString("%1").arg(pos().y()));
//    xml->writeAttribute(tr("z"),QString("%1").arg(zValue()));
...
void GraphicsLineItem::addPoint(const QPointF &point)
{
    if(m_points.count()<2) //the line points must limit to 2
    {
        m_points.append(mapFromScene(point));
        int dir = m_points.count();
        SizeHandleRect *shr = new SizeHandleRect(this, dir+Left, dir == 1 ? false : true);
...
PointF  GraphicsBezier::bezier_interpolation_func(double t, PointF *points)
{
    int siz=m_points.count();
    for (int i = 1; i < siz; ++i)
    {
...
void  GraphicsBezier::draw_bezier_curves(PointF *points, PointF *out_points,int num)
{
    double step = 1.0/ num;
    double t = 0.;
    for (int i = 0; i < num; i++)
    {
        PointF temp_point = bezier_interpolation_func(t, points);
        t += step;
        out_points[i] = temp_point;
    }
}
bool GraphicsBezier::saveToXml_Ex(QXmlStreamWriter *xml)
{
    if ( m_isBezier )
    {
        xml->writeStartElement("FileMusic");
        xml->writeAttribute(tr("src"),QString("%1").arg(""));
        xml->writeEndElement(); //FileMusic
        xml->writeStartElement("motorAction");
        xml->writeAttribute(tr("id"),QString("%1").arg(this->m_servoid));
        int cnt=m_points.count();
        double index_time=TIME_MAX/SCENE_WIDTH;
        double index_angle=ANGLE_MAX/SCENE_HEIGHT;
        QVariant t(qAbs(m_points[m_points.count()-1].x()-m_points[0].x())*index_time*1000/div_time);
        int time_span = t.toInt();//data number;
        QPointF f;
        PointF *in = (PointF *)malloc(cnt* sizeof(PointF));
        for (int i = 0; i <cnt; i++)
        {
            f= this->mapToScene(m_points[i]);
            in[i].X = f.x();
            in[i].Y = f.y();
        }
        PointF out[time_span];
        draw_bezier_curves(in, out,time_span);
        free(in);
        for(int i=0;i<time_span;++i)
        {
            xml->writeStartElement("mPosition");
            xml->writeAttribute(tr("X"),QString("%1").arg("1"));
            double y=(out[i].Y=index_angle*out[i].Y);
...
//    //    xml->writeAttribute(tr("actionGroupID"),QString("%1").arg(2));
//    //    xml->writeAttribute(tr("servoID"),QString("%1").arg(1));
//    /*------根据四个点和开始结束时间生成命令中需要的点---------------------------------------*/
//    //1.转换坐标系统，将当前的坐标转换为x方向的时间坐标和y坐标的角度

//    int cnt=m_points.count();
//    double index_time=TIME_MAX/SCENE_WIDTH;
//    double index_angle=ANGLE_MAX/SCENE_HEIGHT;
//    QVariant t(qAbs(m_points[m_points.count()-1].x()-m_points[0].x())*index_time*1000/div_time);
//    int time_span = t.toInt();//需要发送的命令也就是点的个数,此变量用来确定bezier 点的个数
//    QPointF f;
//    PointF *in = (PointF *)malloc(cnt* sizeof(PointF));
//    for (int i = 0; i <cnt; i++)
//    {
...
//    //    double index_time=TIME_MAX/SCENE_WIDTH;
//    //    double index_angle=ANGLE_MAX/SCENE_HEIGHT;
//    //需要一次转换，从图元坐标转换为角度和时间值
//    for(int i=0;i<time_span;++i)
//    {
//        xml->writeStartElement("parameter");
//        xml->writeAttribute(tr("actionGroupID"),QString("%1").arg(m_actionid));
//        xml->writeAttribute(tr("servoID"),QString("%1").arg(m_servoid));
//        double x=(out[i].X=index_time*out[i].X);
//        double y=(out[i].Y=index_angle*out[i].Y);
//        xml->writeAttribute("time",QString("%1").arg(x));
//        xml->writeAttribute("angle",QString("%1").arg(y));
//        xml->writeEndElement();
//    }
//    /*----------------------------------------------------------------------*/
//    xml->writeEndElement();
...
void GraphicsBezier::slt_fill_bezier_item(QVector<QPointF>v)
{
    qDebug("received from scene v.size:%d\n",v.size());
    if(v.size()==4)
    {
        isFill=true;
        temp.clear();
...
{
    m_points.append(mapFromScene(point));//save the point for itself;
    int dir = m_points.count();//save point count;
...
//        for(int i=0;i<m_points.count();++i)
//        {
//            qDebug("times:%d m_point[%d],x=%.2f,y=%.2f\n",cnt,i,m_points[i].x(),m_points[i].y());
//        }
        m_localRect = m_points.boundingRect();
        m_width = m_localRect.width();
        m_height = m_localRect.height();
...
void GraphicsPolygonItem::endPoint(const QPointF & point)
{
    ...
    if( nPoints > 2 && (m_points[nPoints-1] == m_points[nPoints-2] ||
                        m_points[nPoints-1].x() - 1 == m_points[nPoints-2].x() &&
                        m_points[nPoints-1].y() == m_points[nPoints-2].y())){
        delete m_handles[Left + nPoints-1]; //如果两个点相等，删除最后一个点。
        m_points.remove(nPoints-1);
