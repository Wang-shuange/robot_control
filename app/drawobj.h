...
public:
    ...
    virtual bool loadFromXml(QXmlStreamReader * xml );
    virtual bool saveToXml( QXmlStreamWriter * xml );
    QString displayName() const { return (m_isBezier? tr("bezier"):tr("polyline")); }
    PointF bezier_interpolation_func(double t, PointF *points);
    void draw_bezier_curves(PointF *points, PointF *out_points, int num);
    bool saveToXml_Ex(QXmlStreamWriter *xml);
public slots:
    void slt_fill_bezier_item(QVector<QPointF> v);
protected:
    void paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget);
private:
    bool m_isBezier;
public:
    bool isFill_with_bezier;
signals:
    void sig_info2scene(bool isPaint);
