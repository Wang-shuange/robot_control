...
//    char            digit_str[2] = { '\0', '\0' };
    ...
    if ( (upper - lower) == 0.0 ) return ;//no value to draw
    increment = static_cast<double>( width / (upper - lower));//unit's pixels value
    scale = static_cast<unsigned int>(ceil (max_size));
...
//           painter->drawLine(m_lastPos.x(),0, m_lastPos.x() , 1000);
//       }
//   }
//}
...
void QtRuleBar::drawPos(QPainter *painter)
{
   ...
   if ((bs_width > 0) && (bs_height > 0)){
       ...
       if (m_direction==Qt::Horizontal){
           ...
           x = qRound ((position - lower) * increment) + bs_width / 2 - 1;
           y = (height + bs_height) / 2 ;
           painter->drawLine(m_lastPos.x(),0, m_lastPos.x() , height+1000);
       }else{
           increment = (double) height / (upper - lower);
           x = (width + bs_width) / 2 ;
           y = qRound ((position - lower) * increment) + (bs_height) / 2 - 1;
           painter->drawLine(0 , m_lastPos.y() , width+1000 , m_lastPos.y());
       }
   }
}
